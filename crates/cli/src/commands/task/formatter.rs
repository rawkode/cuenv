...
/// Execute multiple tasks with formatter support
pub async fn execute_tasks_with_formatter(
    executor: &TaskExecutor,
    task_names: &[String],
    args: &[String],
    audit: bool,
    ...
    match output_format {
        "spinner" => {
            execute_tasks_spinner(executor, task_names, args, audit, &mut shutdown_rx).await
        }
        "tui" => {
            // Check if we're in a TTY environment
            if !atty::is(atty::Stream::Stderr) {
                ...
                execute_tasks_tui(executor, task_names, args, audit, &mut shutdown_rx).await
            }
        }
        _ => {
            execute_tasks_simple(
                executor,
                task_names,
                args,
                audit,
...
async fn execute_tasks_tui(
    ...
    let task_handle = tokio::spawn(async move {
        ...
        // Execute the tasks with output capture
        executor_clone
            .execute_tasks_with_capture(&task_names_clone, &args_clone, audit)
            .await
    });
    ...
    let task_result = match task_handle.await {
        Ok(result) => result,
        Err(_) => Ok(1), // Tasks were cancelled or panicked
    };
...
async fn execute_tasks_spinner(
    executor: &TaskExecutor,
    task_names: &[String],
    args: &[String],
    audit: bool,
    ...
    let plan = executor.build_execution_plan(task_names)?;

    // Create and initialize spinner formatter
    let mut formatter = SpinnerFormatter::new(task_registry.clone());
    formatter
        .initialize(&plan)
        .await
    ...
    // Create formatter Arc first
    let formatter_arc = Arc::new(formatter);

    // Create a bridge to forward core events to spinner formatter
    let formatter_for_bridge = formatter_arc.clone();
    let bridge_handle = tokio::spawn(async move {
        let core_bus = cuenv_core::events::global_event_bus();
        let mut subscriber = core_bus.subscribe();
    ...
    // Execute tasks with dependencies and OUTPUT CAPTURE
    let result = tokio::select! {
        result = executor.execute_tasks_with_capture(
            task_names,
            args,
            audit
        ) => result,
        _ = shutdown_rx.recv() => {
            // Mark all running/queued tasks as cancelled
            for task_name in task_names {
                task_registry.update_task_state(task_name, TaskState::Cancelled).await;
            }
            eprintln!("Task execution cancelled");
...
async fn execute_tasks_simple(
    ...
    println!("Executing {} tasks", task_names.len());

    // Execute with cancellation support
    tokio::select! {
        result = executor.execute_tasks_with_dependencies(task_names, &[], audit) => {
            match result {
                Ok(0) => println!("✓ All tasks completed successfully"),
                Ok(code) => eprintln!("✗ Tasks failed with exit code: {code}"),
                Err(ref e) => eprintln!("✗ Tasks failed: {e}"),
            }
            result
        },
        _ = shutdown_rx.recv() => {
            eprintln!("\n⚠️  Tasks cancelled by user");
            Ok(130)
        }
    }
}
