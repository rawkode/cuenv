...
async fn main() -> eyre::Result<()> {
    ...
    cuenv_utils::cleanup::init_cleanup_handler();
    ...
    if let Some(cache_mode) = &cli.cache {
        let _mode = match cache_mode.as_str() {
            ...
            _ => {
                eyre::bail!("Invalid cache mode: {cache_mode}")
            }
        };
...
"#;
                ...
                Some(name) => {
                        ...
                        && crate::monorepo::is_monorepo(&current_dir)
                    {
                        ...
                        let status = if audit {
                            ...
                            // For now, use basic execution until TUI is properly integrated
        ...
        Some(Commands::Cache { command }) => {
            // For now, provide basic cache operations without the complex cache system
            match command {
                CacheCommands::Clear => {
                    // Basic cache clear - remove .cache directory if it exists
                    ...
                    if cache_dir.exists() {
                        ...
                        println!("✓ Cache directory doesn't exist, nothing to clear");
                    }
                }
                CacheCommands::Stats => {
                    ...
                    if cache_dir.exists() {
                        ...
                        match walkdir::WalkDir::new(cache_dir).into_iter().count() {
                            count if count > 1 => println!("  Cache entries: {} files", count - 1), // Subtract 1 for the directory itself
                            _ => println!("  Cache entries: 0 files"),
                        }
                    } else {
                        println!("  Cache entries: 0 files");
                    }
                    println!("  Note: Full statistics require the advanced cache system");
                }
                CacheCommands::Cleanup { max_age_hours: _ } => {
                    println!("Note: Basic cleanup not implemented.");
                    println!("Use 'cuenv cache clear' to remove all cache entries.");
                }
            }
        }
