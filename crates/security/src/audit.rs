    ...
    pub fn new(config: AuditConfig) -> Result<Self> {
        ...
        let writer: Option<Box<dyn Write + Send>> = if let Some(ref path) = config.log_file {
            let file = OpenOptions::new()
                ...
                .append(true)
                .open(path)
                .map_err(|e| cuenv_core::Error::file_system(path.clone(), "open", e))?;
            Some(Box::new(file))
        } else {
            None
        };
    ...
    async fn write_entry(&self, entry: &AuditEntry) -> Result<()> {
        ...
        if let Some(ref mut w) = *writer {
            writeln!(w, "{json}").map_err(|e| {
                cuenv_core::Error::file_system(PathBuf::from("audit.log"), "write", e)
            })?;
            w.flush().map_err(|e| {
                cuenv_core::Error::file_system(PathBuf::from("audit.log"), "flush", e)
            })?;
        } else {
...
pub fn init_audit_logger(config: AuditConfig) -> Result<()> {
    ...
    AUDIT_LOGGER.set(logger).map_err(|_| {
        cuenv_core::Error::configuration("Audit logger already initialized".to_string())
    })?;
    Ok(())
}
