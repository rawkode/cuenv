...
impl<T: Cache> SecureCacheBuilder<T> {
    ...
    pub async fn build(self) -> Result<SecureCache<T>> {
        ...
        let authority =
            crate::capabilities::CapabilityAuthority::new("cuenv-secure-cache".to_string());
...
impl<T: Cache> SecureCache<T> {
    ...
    async fn authorize_operation(
        ...
        match result {
            ...
            AuthorizationResult::TokenInvalid(reason) => Err(CacheError::InvalidToken {
                ...
                reason: match reason {
                    crate::capabilities::TokenVerificationResult::Expired => {
                        TokenInvalidReason::Expired
                    }
                    crate::capabilities::TokenVerificationResult::Revoked => {
                        TokenInvalidReason::Revoked
                    }
                    crate::capabilities::TokenVerificationResult::InvalidSignature => {
                        TokenInvalidReason::InvalidSignature
                    }
                    crate::capabilities::TokenVerificationResult::InvalidIssuer => {
                        TokenInvalidReason::UntrustedIssuer
                    }
                    crate::capabilities::TokenVerificationResult::InvalidPublicKey => {
                        TokenInvalidReason::InvalidSignature
                    }
        ...
        if !report.tree_valid {
            ...
            self.audit_logger
                .log_security_violation(
                    crate::audit::SecurityViolationType::IntegrityFailure,
                    format!(
                        "Merkle tree integrity failure: {} corrupted entries",
                        report.corrupted_entries.len()
                    ),
                    crate::audit::ViolationSeverity::High,
                    context,
                )
                .await?;
        }
    ...
    pub async fn get_merkle_proof(&self, key: &str) -> Result<Option<crate::merkle::MerkleProof>> {
        if !self.config.enable_merkle_tree {
            return Ok(None);
        }
...
impl<T: Cache + Send + Sync> Cache for SecureCache<T> {
    ...
    async fn remove(&self, key: &str) -> Result<bool> {
        ...
        self.audit_logger
            .log_event(
                crate::audit::AuditEvent::CacheDelete {
                    key: key.to_string(),
                    existed: result,
                    duration_ms,
                },
    ...
    async fn clear(&self) -> Result<()> {
        ...
        self.audit_logger
            .log_event(
                crate::audit::AuditEvent::CacheClear {
