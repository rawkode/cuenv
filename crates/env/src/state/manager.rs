...
use cuenv_security::audit_logger;
use cuenv_utils::compression;
use cuenv_utils::sync::SyncEnv;
...
impl StateManager {
    ...
    fn encode_and_store<T: Serialize>(
        ...
        let encoded = compression::encode(value).with_context(|| context.to_string())?;
    ...
    fn decode_from_var<T: for<'de> Deserialize<'de>>(
        ...
        match SyncEnv::var(key)? {
            Some(encoded) => {
                let decoded = compression::decode(&encoded).with_context(|| context.to_string())?;
                Ok(Some(decoded))
            }
            None => Ok(None),
        }
