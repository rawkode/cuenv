...
use cuenv_config::{Config, CommandConfig, TaskConfig, Hook, VariableMetadata};
...
impl EnvManager {
    ...
    pub fn export_for_shell(&self, shell: &str) -> Result<String> {
        let shell_enum = match shell {
            ...
            _ => return Err(Error::configuration(format!("Unsupported shell: {}", shell))),
        };
    ...
    pub fn run_command(&self, command: &str, args: &[String]) -> Result<i32> {
        if let Some(cmd_config) = self.config.get_command(command) {
            ...
            let status = cmd.status()
                .map_err(|e| Error::command_execution(format!("Failed to execute command '{}': {}", command, e)))?;
    ...
    pub fn run_command_with_restrictions(
        ...
        let status = cmd.status()
            .map_err(|e| Error::command_execution(format!("Failed to execute restricted command '{}': {}", command, e)))?;
    ...
    pub fn get_command_capabilities(&self, command: &str) -> Vec<String> {
        self.config.get_command(command)
        ...
        for (name, value) in &resolved_vars {
            if let Some(metadata) = self.config.get_variable_metadata(name) {
                if metadata.capabilities.is_empty() || 
                   capabilities.iter().any(|cap| metadata.capabilities.contains(cap)) {
    ...
    fn execute_hook(&self, hook: &Hook) -> Result<()> {
        ...
        match hook {
            Hook::Legacy(config) => {
                ...
                let status = cmd.status()
                    .map_err(|e| Error::command_execution(format!("Hook execution failed: {}", e)))?;
                
                if !status.success() {
                    return Err(Error::command_execution("Hook returned non-zero exit code".to_string()));
                }
            }
            Hook::Exec { exec, .. } => {
    ...
    fn run_command_config(&self, cmd_config: &CommandConfig, args: &[String]) -> Result<i32> {
        ...
        let capabilities = cmd_config.capabilities.as_ref().map(|c| c.as_slice()).unwrap_or(&[]);
        ...
        let status = cmd.status()
            .map_err(|e| Error::command_execution(format!("Failed to execute command '{}': {}", cmd_config.command, e)))?;
    ...
    fn create_test_config() -> Arc<Config> {
        let config = Config::new(
            ...
            cuenv_config::ParseResult {
                variables: [("TEST_VAR".to_string(), "test_value".to_string())].into_iter().collect(),
