use anyhow::Context;
use cuenv_config::{DevenvConfig, ExecConfig, NixFlakeConfig};
...
pub async fn execute_nix_flake_hook(
    ...
    if !force_reload {
        if let Ok(cached_env) = cache.load() {
            ...
            let watch_files =
                cuenv_env::default_watch_files(Path::new(flake.dir.as_deref().unwrap_or(".")));
            let watcher = cuenv_env::FileWatcher::new(watch_files);
...
pub async fn execute_devenv_hook(
    ...
    if !force_reload {
        if let Ok(cached_env) = cache.load() {
            let watch_files =
                cuenv_env::default_watch_files(Path::new(devenv.dir.as_deref().unwrap_or(".")));
            let watcher = cuenv_env::FileWatcher::new(watch_files);
