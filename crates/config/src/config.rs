...
impl ConfigLoader {
    ...
    fn build_parse_options(&self) -> ParseOptions {
        ...
        options.environment = self.cli_options.environment.clone()
...
mod tests {
    ...
    fn test_config_loader_basic() {
        ...
        assert_eq!(config.variables.get("DATABASE_URL"), Some(&"postgres://localhost/test".to_string()));
        assert_eq!(config.variables.get("API_KEY"), Some(&"test-key".to_string()));
    ...
    fn test_config_loader_with_environment() {
        ...
        assert_eq!(config.variables.get("DATABASE_URL"), Some(&"postgres://prod.example.com/db".to_string()));
    ...
    fn test_config_loader_missing_file() {
        ...
        assert!(result.unwrap_err().to_string().contains("No env.cue file found"));
    }
