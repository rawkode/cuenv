...
use cuenv_env::manager::environment::preload_supervisor::{
    run_supervisor, CapturedEnvironment, PreloadSupervisor
};
...
"#
    ...
    assert_eq!(captured.env_vars.get("TEST_VAR1"), Some(&"value1".to_string()));
    assert_eq!(captured.env_vars.get("TEST_VAR2"), Some(&"value2".to_string()));
    assert_eq!(captured.env_vars.get("TEST_VAR3"), Some(&"value3".to_string()));
}
...
"#
    ...
    assert!(!marker_file.exists(), "Script should NOT have been executed (cache hit)");
    ...
    assert!(marker_file.exists(), "Script should have been executed (cache miss)");
}
...
async fn test_supervisor_handles_hook_failures_gracefully() {
    ...
    assert!(result.is_ok(), "Supervisor should complete despite hook failures");
}
...
async fn test_supervisor_timeout_handling() {
    ...
    let result = timeout(Duration::from_secs(2), run_supervisor(hooks)).await;
...
async fn test_supervisor_mixed_preload_and_regular_hooks() {
    ...
    assert!(result.is_ok(), "Supervisor should filter and run only preload hooks");
}
