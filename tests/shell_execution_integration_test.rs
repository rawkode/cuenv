...
fn test_bash_hook_syntax_validation() {
    ...
    let hook_content = shell.generate_hook(cuenv_path).expect("Failed to generate bash hook");
...
fn test_zsh_hook_syntax_validation() {
    ...
    let hook_content = shell.generate_hook(cuenv_path).expect("Failed to generate zsh hook");
...
fn test_fish_hook_syntax_validation() {
    ...
    let hook_content = shell.generate_hook(cuenv_path).expect("Failed to generate fish hook");
...
fn test_hook_contains_required_functions() {
    let bash = Bash::new();
    let bash_hook = bash.generate_hook("/usr/local/bin/cuenv").expect("Failed to generate bash hook");
    
    // Bash hooks should contain essential functions
    assert!(bash_hook.contains("cuenv"), "Bash hook should reference cuenv command");
    assert!(bash_hook.contains("cd") || bash_hook.contains("chdir"), 
        "Hook should handle directory changes");
    
    // Test zsh hook
    let zsh = Zsh::new();
    let zsh_hook = zsh.generate_hook("/usr/local/bin/cuenv").expect("Failed to generate zsh hook");
    
    assert!(zsh_hook.contains("cuenv"), "Zsh hook should reference cuenv command");
    
    // Test fish hook
    let fish = Fish::new();
    let fish_hook = fish.generate_hook("/usr/local/bin/cuenv").expect("Failed to generate fish hook");
    
    assert!(fish_hook.contains("cuenv"), "Fish hook should reference cuenv command");
}
...
fn test_hook_generation_with_different_paths() {
    ...
    for path in paths {
        ...
        assert!(hook_content.is_ok(), "Should generate hook for path: {}", path);
        
        let content = hook_content.unwrap();
        assert!(content.contains(path), "Hook should contain the specified cuenv path: {}", path);
    ...
    for path in problematic_paths {
        ...
        if path.is_empty() {
            ...
            if result.is_ok() {
                ...
                assert!(!hook.is_empty(), "Hook should not be empty even for empty path");
            }
        } else {
            // Other paths should generally succeed
            assert!(result.is_ok(), "Should handle path with special characters: {}", path);
            
            if result.is_ok() {
                let hook = result.unwrap();
                assert!(!hook.is_empty(), "Generated hook should not be empty for path: {}", path);
            }
        }
    }
}
...
fn test_shell_session_simulation() {
    ...
    let hook_content = bash.generate_hook("/usr/local/bin/cuenv").expect("Failed to generate bash hook");
...
"#,
    ...
    if let Ok(result) = output {
        if result.status.success() {
            ...
            println!("Shell session test failed (this may be expected in CI): {}", stderr);
        }
    } else {
...
fn test_hook_generation_consistency() {
    ...
    let hooks: Vec<String> = (0..5)
        .map(|_| bash.generate_hook(cuenv_path).expect("Failed to generate hook"))
        .collect();
    
    // All hooks should be identical
    for (i, hook) in hooks.iter().enumerate().skip(1) {
        assert_eq!(&hooks[0], hook, 
            "Hook generation should be consistent. Hook {} differs from hook 0", i);
    }
}