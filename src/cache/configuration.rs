...
pub enum TaskCacheConfig {
    ...
    Advanced {
        ...
        env: Option<CacheKeyFilterConfig>,
    },
}
...
impl TaskCacheConfig {
    ...
    pub fn env_filter(&self) -> Option<&CacheKeyFilterConfig> {
        match self {
...
impl CacheConfigLoader {
    ...
    fn load_from_config_file() -> Result<Option<CacheConfiguration>> {
        ...
        let content = std::fs::read_to_string(&config_path)
            .map_err(|e| Error::file_system(&config_path, "read config file", e))?;

        let file_config: serde_json::Value =
            serde_json::from_str(&content).map_err(|e| Error::Json {
                message: format!("Failed to parse config file: {}", config_path.display()),
                source: e,
            })?;
...
impl CacheConfigResolver {
    ...
    pub fn get_task_env_filter(
        ...
        if let Some(task_filter) = task_config.and_then(|c| c.env_filter()) {
...
mod tests {
    ...
    fn test_task_cache_config() {
        ...
        let advanced_config = TaskCacheConfig::Advanced {
            enabled: false,
            env: Some(CacheKeyFilterConfig::default()),
        };
